...
| R21 | Android VPN: на подключении manual A → действие уведомления «Перевыбор» → OFF → ON без выбора сервера → трафик | Сразу после действия и после повторного подключения выбран Auto, native selector разрешается в конкретный leaf, свежие HTTPS/MTProto идут через VPN; Windows N/A |
...
R21 проверяет именно действие уведомления при живом VPN. Выбор «Автовыбор» из списка
серверов вместо «Перевыбора» не воспроизводит этот путь; после действия нельзя
повторно выбирать сервер до проверки подключения и сохранённого намерения.
