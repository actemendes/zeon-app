...
| Этап 05 | Частичный результат 14.09.2026 на `1.5.0+1050016` / `984d40c4`: точный R08 PASS на физическом Android VPN с двусторонним Telegram UID flow, двумя HTTPS и MTProto DC1/DC2; Windows System Proxy и контрольные TUN/Local Proxy PASS. Этап не закрыт: R13 `BLOCKED_BY_FIXTURE`, потому что изолированной initialized fault+B fixture нет; production endpoints не изменялись |
...
| 05 / 6a9fbc7a8f08c18d5f58c270 | R08 исправлен и PASS 14.09.2026 на `1.5.0+1050016` / `984d40c4`; задача остаётся незавершённой из-за `BLOCKED_BY_FIXTURE` для R13 | Android Telegram UID flow + HTTPS/MTProto и Windows три режима PASS; нужен isolated initialized fault+B endpoint для R13 |
