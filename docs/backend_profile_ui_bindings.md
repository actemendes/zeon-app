...
- Канонический `open`-маршрут: `https://api.zeon-vps.online/open/$openId` (primary).
...
8. Для `open`-ввода canonical URL в профиле остается primary (`api.zeon-vps.online`), даже если источник ввода был public-ссылка.
