...
Сверять A/B с актуальным профилем после его штатного обновления: старый backup
может содержать уже отсутствующие tags/адреса. Ожидаемый выход нельзя подменять
наблюдаемым значением неудачного теста. Во время device run телефон принадлежит
тестовому исполнителю; ручной выбор сервера/подключение нарушает исходные условия.
Если следующий run не видит disconnected baseline, он BLOCKED и не останавливает
соединение, которым не владеет. После явной передачи устройства владелец восстанавливает
исходное состояние, затем запускается новый run; старый результат не переписывается.
