    ...
    if ($report.active_run_present) {
        ...
        if ($report.deadline_exceeded -or $report.heartbeat_stale) {
            $report.planned_actions = @(
                ...
                'restore the run-recorded SYSTEM WinINet and WinHTTP baseline',
            ...
            if (Test-Path -LiteralPath $armingPath) {
                ...
                $report.arming_valid = [string]$arming.run_id -ceq $runId -and [bool]$arming.allow_apply -and -not [bool]$arming.consumed -and [string]$arming.active_run_path -ceq $activePath -and [datetime]::Parse([string]$arming.expires_at).ToUniversalTime() -ge $now
            }
