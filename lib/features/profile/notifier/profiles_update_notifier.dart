...
class ForegroundProfilesUpdateNotifier extends _$ForegroundProfilesUpdateNotifier with AppLogger {
  ...
  Future<void> updateProfiles() async {
    ...
    try {
      // Managed routing and remote profiles form one freshness transaction.
      // Fetch both policy bundles first so every config generated by this cycle
      // sees one freshness boundary. The services remain fail-open and preserve
      // their LKG if the control plane is temporarily unavailable.
      await Future.wait([
        ref.read(managedRuleSetSyncServiceProvider).sync(force: true, reason: 'foreground_profile_cycle'),
        ref.read(managedApplicationSyncServiceProvider).sync(force: true, reason: 'foreground_profile_cycle'),
      ]);
