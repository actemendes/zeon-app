...
struct NotificationActivationCallback :
  ...
  HRESULT __stdcall Activate(
    ...
    try {
      ...
      const auto payload = WideToUtf8(args);
...
DWORD RegisterCallback(const std::string& guid, NativeNotificationCallback callback) {
  DWORD registration {};
...
void NativePlugin::reset() noexcept {
  ...
  notifier.reset();
  history.reset();
  if (callbackRegistration != 0) {
    CoRevokeClassObject(callbackRegistration);
...
bool NativePlugin::registerApp(
  ...
  callbackRegistration = RegisterCallback(guid, callback);
  return callbackRegistration != 0;
}
