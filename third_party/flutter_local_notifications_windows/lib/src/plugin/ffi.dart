...
void _globalLaunchCallback(NativeLaunchDetails details) {
  FlutterLocalNotificationsWindows.instance?._onDidReceiveNotificationResponse(details);
}
...
class FlutterLocalNotificationsWindows extends WindowsNotificationsBase {
  ...
  Future<void> cancel({required int id}) async {
    if (!_isReady) {
      throw StateError('Flutter Local Notifications must be initialized before use');
    }
  ...
  Future<void> show({
    ...
    final Map<String, String> bindings = <String, String>{
      ...
      for (final WindowsProgressBar progressBar in notificationDetails?.progressBars ?? <WindowsProgressBar>[])
        ...progressBar.data,
    };
    ...
    final bool result = _bindings.showNotification(_plugin, id, xml.toNativeUtf8(allocator: arena), nativeMap);
    if (!result) {
      throw Exception('Flutter Local Notifications could not show notification');
    }
  });
  ...
  @override
  Future<void> zonedScheduleRawXml({required int id, required String xml, required TZDateTime scheduledDate}) async =>
      using((Arena arena) {
        if (!_isReady) {
          throw StateError('Flutter Local Notifications must be initialized before use');
        }
  ...
  @override
  Future<NotificationUpdateResult> updateBindings({required int id, required Map<String, String> bindings}) async =>
      using((Arena arena) {
        if (!_isReady) {
          throw StateError('Flutter Local Notifications must be initialized before use');
        }
